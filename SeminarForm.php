...
if (!isset($_COOKIE['Index'])){
	...
	//stuff
	if (!empty($_POST['checkbox'])){ #... Update
		for ($i=0; $i < count($_POST['checkbox']); $i++) {
			$formData["$i"] =  $_POST['checkbox'][$i];
		}
	}
	else {
		$errors++;
		$body .= "You have not selected any seminars. You will select a seminar.";
	}
	
	/*Unique Validation*/
	if ($errors == 0){
		//Controlled responce, validation unneeded
	}
	
	/*Save information to cookie and link to leave*/
	if ($errors == 0) {
		for ($i=0; $i < count($_POST['checkbox']); $i++) {
			setcookie("SeminarForm[$i]", $formData[$i], time() + 3600);
		}
		$body .= "<a href='ReviewRegistration.php'>Continue</a><br />\n"; #!!!! No HREF is set
	}
